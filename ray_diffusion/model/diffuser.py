...
class RayDiffuser(nn.Module):
    def __init__(
        ...
        self.distortion_predictor = ViTMAEForPreTraining.from_pretrained("facebook/vit-mae-base")
        for param in self.distortion_predictor.vit.parameters():
            param.requires_grad = False 
        # configuration = ViTMAEConfig(image_size=448)
        # self.distortion_predictor = ViTMAEForPreTraining(configuration)
    ...
    def forward_rect(self, rays):
        """
        Predicts the distortion control point grids of the input rays images.
        """
        # B, S, C, H, W = rays.shape
        # rays = rays.reshape(B*S, C, H, W)
        # rays_interp = torch.nn.functional.interpolate(rays, size=(224, 224), mode="bicubic")
        # distort_pred = self.distortion_predictor(rays_interp)
        # return distort_pred.reshape(B, S, 2, H, W)
        B, S, C, H, W = rays.shape
        rays = rays.reshape(B*S, C, H, W)
        rays_interp = torch.nn.functional.interpolate(rays, size=(224, 224), mode="bicubic")
        outputs = self.distortion_predictor(rays_interp[:, 3:, :, :])
        logits = self.distortion_predictor.unpatchify(outputs.logits).reshape(B, S, C//2 , 224, 224)[:, :, :2, :, :]
        logits = torch.nn.functional.interpolate(logits.reshape(B*S, 2, 224, 224), size=(448, 448), mode="bicubic").reshape(B, S, 2, 448, 448)
        return logits