...
def create_training_visualizations(
    ...
    for index in range(len(images)):
        pred_cameras = []
        per_sample_images = []
        flows_sample = flows[index] if flows is not None else None
        for ii in range(num_images):
            ...
            #######
            diff_rays = rays_gt.get_directions()[ii] - rays.get_directions()[ii]
            vis = diff_rays + 0.5
            axs[1, -1].imshow(vis.reshape(num_patches_y, num_patches_x, 3).cpu())
            axs[1, -1].set_title("Diff Directions")

            vis = torchvision.utils.flow_to_image(flows_sample[ii].to(torch.float32)) / 255.
            axs[0, -1].imshow(vis.permute(1, 2, 0).cpu())
            axs[0, -1].set_title("Pred Flow")
            #######
