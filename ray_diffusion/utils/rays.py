...
def compute_ndc_coordinates(
    ...
    xyd_grid = torch.stack([x, y], dim=-1)
    if flow is not None:
        small_flow = torch.nn.functional.interpolate(flow[0, :, :, :].unsqueeze(0), size=(num_patches_y, num_patches_x), mode='bilinear', align_corners=False)
        small_flow = small_flow.squeeze(0).permute(1, 2, 0)
        xyd_grid = xyd_grid - small_flow
        xyd_grid[:, :, 0] = xyd_grid[:, :, 0] * width / 2 - cc_x
        xyd_grid[:, :, 1] = xyd_grid[:, :, 1] * width / 2 - cc_y
    
    xyd_grid = torch.cat([xyd_grid, torch.ones_like(xyd_grid[..., 0:1])], dim=-1)
    return xyd_grid
