...
import kornia 
from ray_diffusion.dataset.distortion_pattern.fisheye import get_grid
import gryds
...
class Co3dDataset(Dataset):
    ...
    def get_data(self, index=None, sequence_name=None, ids=(0, 1), no_images=False):
        ...
        if aug_type == "barrel":
            ...
            gamma = torch.tensor([10, 0.1])
            aug_forms = AugmentationSequential(
                kornia.augmentation.RandomFisheye(center_x, center_y, gamma, p=1.,keepdim=True),
                kornia.augmentation.CenterCrop(size=(int(self.img_size*0.7), int(self.img_size*0.7)), keepdim=True),
            ...
            aug_forms = AugmentationSequential(
                kornia.augmentation.RandomFisheye(center_x, center_y, gamma, p=1.,keepdim=True),
                kornia.augmentation.CenterCrop(size=(int(self.img_size*0.6), int(self.img_size*0.6)), keepdim=True),
                kornia.augmentation.Resize(size=(self.img_size, self.img_size), keepdim=True),
                same_on_batch=False)
        ...
        for anno in annos:
            ...
            if not no_images:
                ...
                # image_distort = image[:, : self.img_size, : self.img_size].permute(1, 2, 0).numpy()

                # # # # setup distortions
                # # # [W, H, 3] -> [W, H, 3] [W, H, 2]
                # if aug_type in ['barrel', 'shear']:
                #     image_distort, flow = augment_distortion(aug_type, image_distort)
                # # elif aug_type in ['pincushion','projective']:
                # #     image_distort, flow = generatepindata(aug_type, image_distort)
                # else:
                #     flow = np.zeros((self.img_size, self.img_size, 2))

                # bspline_trans = gryds.BSplineTransformation(distort_grid.reshape([2, 5, 5]))
                # interpreter = gryds.MultiChannelInterpolator(image_distort, order=3, cval=[1, 1, 1])
                # transformed = interpreter.transform(bspline_trans)
                # image_distort = torch.from_numpy(image_distort).permute(2, 0, 1)

                image_distort = aug_forms(image.unsqueeze(0), params=aug_forms._params).squeeze()
